...
for iter_test in range(itre):
    ...
    if config.datastes_type == 'not_real':
        if config.input_type == 'video':     #有GT是video
            ...
            img4 = Image.open(test_dataset[iter_test][4]).convert("RGB")
            img5 = Image.open(test_dataset[iter_test][5]).convert("RGB")
    ...
    if config.datastes_type == 'not_real':
        if config.input_type == 'video':     #有GT是video
            ...
            temp1 = torch.stack((img_haze[0][6], img_haze[0][7], img_haze[0][8]), 0)
            temp1 = temp1.unsqueeze(0)
            img_haze = temp1
