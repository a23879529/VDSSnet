...
def get_parser():
    ...
    parser.add_argument('--config', type=str, default='./semantic_segmentation/config/ade20k/ade20k_pspnet101.yaml', help='config file')
    #.add_argument('--image', type=str, default='C:/Users/ian/Desktop/data', help='input image dir')
    parser.add_argument('opts', help='see ./semantic_segmentation/config/ade20k/ade20k_pspnet101.yaml for all options', default=None, nargs=argparse.REMAINDER)
...
def main():
    ...
    if args.arch == 'psp':
        from semantic_segmentation.model.pspnet import PSPNet
        model = PSPNet(layers=args.layers, classes=args.classes, zoom_factor=args.zoom_factor, pretrained=False)
    elif args.arch == 'psa':
        from semantic_segmentation.model.psanet import PSANet
    ...
    return model
    #print(args.image)
    # image_name_list = glob.glob(r'C:\\Users\\ian\\Desktop\\data\\*.jpg')    #取得資料夾下所有.jpg圖片
    # for image in image_name_list:
        #print(image)
        # test(model.eval(), image, args.classes, mean, std, args.base_size, args.test_h, args.test_w, args.scales, colors)  #改這裡 修成for迴圈 依序輸進圖片

def demo(model, image):
    global args, logger
    args = get_parser()
    check(args)
    ...
    return test(model.eval(), image, args.classes, mean, std, args.base_size, args.test_h, args.test_w, args.scales, colors)  #改這裡 修成for迴圈 依序輸進圖片
...
def test(model, image_path, classes, mean, std, base_size, crop_h, crop_w, scales, colors):
    ...
    image = image.transpose((2, 1, 0)) # (通道，h，w)->(w，h, 通道)
    ...
    #print(image_path)
    #print(image_name)
    #gray_path = os.path.join('./figure/demo/', image_name + '_gray.png')   #改存圖位置
    #color_path = os.path.join('./figure/demo/', image_name + '_color.png')   #這也要改位置
    #gray_path = "C:\\Users\\田鼠\\Desktop\\ss\\" + image_name + '_gray.png'  
    # color_path = "C:\\Users\\ian\\Desktop\\ss\\" + image_name + '_color.jpg'

    #print(save_folder + image_name + '_color.png')
    ...
    color = loader(color).unsqueeze(0)
    # print(color.shape)
    ...
    # print(color.type())
    # print(color.shape)
    color = torch.transpose(color, 3, 2) # (1, 通道, w，h)->(1, 通道, h, w)
    # print(color.shape)
