...
parser.add_argument('--network', default='DehazeNet')
...
opt.model = 'models/wacv_dehazenet_%s.pth' % opt.task 
opt.use_cuda = opt.gpu_id >= 0
if not os.path.exists(opt.outdir):
    os.makedirs(opt.outdir)
test_img_paths = make_dataset(opt.indir)

if opt.network == 'DehazeNet':
    from Dehaze import DehazeNet
    net = DehazeNet(in_c=4, out_c=3, only_residual=opt.only_residual)
else:
